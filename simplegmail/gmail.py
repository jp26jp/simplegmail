...
class Gmail(object):
    ...
    def __init__(self, email) -> None:
        ...
        service_account_file = settings.GOOGLE_SERVICE_ACCOUNT_CREDENTIALS
        self.creds = service_account.Credentials.from_service_account_file(
            service_account_file, scopes=scopes
        )
        delegated_credentials = self.creds.with_subject(self.delegate_email)
    ...
    def get_unread_inbox(
        ...
        Args:
            ...
            attachments: Accepted values are 'ignore' which completely
                ignores all attachments, 'reference' which partials attachment
    ...
    def list_labels(self, user_id: str = "me") -> List[Label]:
        ...
        try:
            ...
            labels = [Label(name=x["name"], id=x["id"]) for x in response["labels"]]
            return labels
    ...
    def _get_messages_from_refs(
        ...
        Args:
            ...
            attachments: Accepted values are 'ignore' which completely ignores
                all attachments, 'reference' which partials attachment
        ...
        def thread_download_batch(thread_num):
            gmail = Gmail(email=self.delegate_email)
        ...
        try:
            ...
            for header in headers:
                if header["name"].lower() == "date":
                    try:
                        date = str(parser.parse(header["value"]).astimezone())
                    except Exception:
                        date = header["value"]
            ...
            for part in parts:
                if part["part_type"] == "plain":
                    if plain_message is None:
                        plain_message = part["body"]
                    else:
                        plain_message += "\n" + part["body"]
                elif part["part_type"] == "html":
                    if html_message is None:
                        html_message = part["body"]
                    else:
                        html_message += "<br/>" + part["body"]
                ...
                for part in payload["parts"]:
                    ret.extend(
                        self._evaluate_message_payload(
                            part, user_id, message_id, attachments
                        )
                    )
            return ret
